...
const LIGHT = {
  bg:"#FFFFFF", surface:"#FFFFFF", card:"#FFFFFF", raised:"#F7F8FA",
  ...
  return (
    <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",position:"relative"}}>
      ...
      {(()=>{
        ...
        // relative offset: negative = ahead of me, positive = behind me
        const dists = members.map(m => (liveStats[m.id]?.dist ?? 0) - myDist);
        const maxAbs = Math.max(...dists.map(Math.abs), 1);
        // self always at 50%; others spread proportionally left (ahead) or right (behind)
        // leave 14% padding on each side so labels/dots don't clip
        const positions = dists.map(d => Math.max(14, Math.min(86, 50 + (d / maxAbs) * 36)));
        ...
        <div style={{flex:1,overflowY:"auto"}}>
          ...
          <div style={{position:"relative",height:340,background:T.mapBg,flexShrink:0}}>
            ...
            <div style={{position:"absolute",top:10,right:46,background:T.isDark?"rgba(8,11,18,.85)":"rgba(255,255,255,.9)",borderRadius:10,padding:"6px 10px",backdropFilter:"blur(6px)",border:`1px solid ${T.border}`,textAlign:"right",zIndex:1000}}>
              <div style={{fontSize:18,fontWeight:800,color:T.accent,fontFamily:"'Space Mono',monospace",lineHeight:1}}>{liveStats[myMemberId]?.speed??0}</div>
...
const DetailScreen = ({ convoy, onBack, onEdit, onDelete, onStartConvoy, authUser }) => {
  ...
  useEffect(() => {
    loadLeaflet().then(async L => {
      ...
      // Must run after every zoom because Leaflet resets pane z-indexes on zoom events.
      const clampPanes = () => {
      ...
      clampPanes();
      map.on("zoom zoomstart zoomend", clampPanes);
